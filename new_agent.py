...
class gpt_agent_group(model_wrapper):
    ...
    def generate(self, initial_prompt, n_discussion_rounds=1):
        ...
        # The fix to max prompt length issues
        summary = self.agents["mediator"].generate(initial_prompt, self.concat_feedback(self.feedback))
        self.feedback = [summary]
...
def main():
    ...
    output_path = f'uncensored-llama-{intention}-discussion-round-{n_discussion_rounds}.jsonl'
    # output_path = f'{model_name}-{intention}-discussion-round-{n_discussion_rounds}.json'
    responses = {}

    length: int = len(red_teams)
    for i, prompt in enumerate(red_teams):
        response = agent_group_instance.generate(prompt, n_discussion_rounds=n_discussion_rounds)
        responses[prompt] = response
        print(f"{i}/{length}\tSuccess")
        write_to_json_incrementally({prompt: response}, output_path)

# with open(output_path, 'w') as f:
#     json.dump(responses, f, indent=4)

def write_to_json_incrementally(data, file_path):
    with open(file_path, 'a') as file:  # 'a' mode for append to file
        json.dump(data, file)
        file.write('\n')  # Newline for each new JSON entry
